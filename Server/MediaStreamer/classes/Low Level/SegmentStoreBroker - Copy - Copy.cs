...
namespace FatAttitude.MediaStreamer.HLS
{
    internal partial class SegmentStoreBroker
    {
        ...
        {
            Request = _request;
            store = new SegmentStore(ID, Request.UseNewerFFMPEG);
            ...
            if (Request.NewLiveTV || Request.UseAudioStreamIndex >= 0 || Request.UseNewerFFMPEG)
            {
        ...
        {
            Deze code vergelijken met origineel van 1 maand geleden
            if (store.HasSegment(index, segmentNumber))
            {
                ...
                bool foo = store.TryGetSegmentByNumber(index, segmentNumber, ref retrievedSegment);
...
//            int difference = (segmentNumber - Runner.AwaitingSegmentNumber);
            int difference = (-segmentNumber + Runner.startAtSegment);
    //            if (difference2 < 0 || (difference2 == 0 && difference < 0)) // requested segment is in past
           ...
           //            if (difference >= NUMBER_OF_SEGMENTS_CONSIDERED_COMING_SOON)
                ...
                //else
                    ...
                    (Request.LiveTV && Request.NewLiveTV && Request.UseNewerFFMPEG && (difference >= NUMBER_OF_SEGMENTS_CONSIDERED_COMING_SOON_LIVETV)))
                    {
