...
namespace RemotePotatoServer
{
    ...
    //    class RequestProcessor : IDisposable
    {
        ...
        private string RecordFromQueueResult = "";

        //http://www.codeproject.com/Articles/11541/The-Simplest-C-Events-Example-Imaginable
        //88888
        public event RestartRPHandler RestartRP;
        public EventArgs e = null;
        public delegate void RestartRPHandler(RequestProcessor m, EventArgs e);
        ...
        public void Run()
        {
            ...
            try
            {
                ...
                //for (int i = 0; i < Request.Headers.Count; ++i)
                //{
                //    if (Request.Headers.Keys[i].Contains("User-Agent"))
                //    {
                //        if (!Request.Headers[i].Contains("Android RMC"))
                //        {
                //            MessageBox.Show("Your android client should be upgraded to the latest version.");
                //        }
                //    }
                //}
                ...
                if (processMoreActions)
                {
                    switch (txtAction)
                    {
                        ...
                        case "recordfromqueue":
                            string notused;
                            foo = RecordFromQueue(out notused);
                            break;
        ...
        private void WebServiceProcess(string action, ref BrowserSender browserSender, ref string PostObjects)
        {
            ...
            try
            {
                ...
                else if (action.StartsWith("xml/getmediastreamingrequest/"))
                {
                    string strStreamerID = action.Replace("xml/getmediastreamingrequest/", "");
                    int streamerID = 0;
                    try
                    {
                        ...
                        {
                            Functions.WriteLineToLogFile(
                                "Warning in xml/getmediastreamingrequest/: Could not parse streamer ID " + strStreamerID);
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    MediaStreamingRequest msr = StreamingManager.Default.GetMediaStreamingRequest(streamerID);
                    if (msr == null)
                    {
                    ...
                    {
                        XMLresponse = XMLHelper.Serialize<MediaStreamingRequest>(msr);
                    }
                ...
                else if (action.StartsWith("xml/mediastream/start/bymediastreamingrequestgetid"))
                {
                    ...
                    bool foo;
                    if (streamRq != null)
                        streamResult2 = StreamingManager.Default.StartStreamer(streamRq, Request.UserHostName, false, out foo);
                    else
                        streamResult2 = new MediaStreamingResult(MediaStreamingResultCodes.NamedError, "Error in streaming request.");
                ...
                else if (action.StartsWith("xml/mediastream/start/bymediastreamingrequest"))
                {
                    ...
                    bool foo;
                    if (streamRq != null)
                        streamResult = StreamingManager.Default.StartStreamer(streamRq, Request.UserHostName, false, out foo);
                    else
                ...
                else if (action.StartsWith("xml/getduration"))
                {
                    action = action.Replace("xml/getduration/", "");
                    ...
                    long duration = (long)FileBrowseExporter.DurationOfMediaFile_OSSpecific(filePath).TotalMilliseconds;
                    XMLresponse = XMLHelper.Serialize<string>("" + duration);
                }
                ...
                //else if (action.StartsWith("livetv/getshowschedule"))        //suggestion as new syntax
                {
                    ...
                    foreach (var tvProg in tvProgs)
                    {
                        ...
                        if (firstshow == 0)
                        {
                            ...
                            showinfo = "Show Title: " + tvProg.Title + " Start time: " +
                                       String.Format("{0:MMM dd yyyy hh:mm:ss tt}", startingTime) + ". Ends at: " +
                                       String.Format("{0:MMM dd yyyy hh:mm:ss tt}", endingTime) + ". Current time: " +
                                       String.Format("{0:O}", currShowTime) + ". Time left: " +
                                       String.Format("{0:c}", durationLeft) + ". Timezone " + localzone.StandardName;
                        ...
                        else
                        {
                            showinfo = "Show Title: " + tvProg.Title + ". Start time: " +
                                       String.Format("{0:MMM dd yyyy hh:mm:ss tt}", startingTime) + ". Ends at: " +
                                       String.Format("{0:MMM dd yyyy hh:mm:ss tt}", endingTime);
                            Functions.WriteLineToLogFile(showinfo);
                            scheduledshows.Add(showinfo);
                        }
                    }
                ...
                else if (action.StartsWith("xml/setstartsegment"))
                {
                    action = action.Replace("xml/setstartsegment=", "");
                    List<string> parts = action.Split('*').ToList();
                    int startSegmentNr = 0;
                    Int32.TryParse(parts[0], out startSegmentNr);
                    int ID = 0;
                    Int32.TryParse(parts[1], out ID);
                    StreamingManager.Default.SetTimeForIndexFile(ID, startSegmentNr);
                }
                else if (action.StartsWith("xml/storestreaminfo"))
                {
                    MediaStreamingResult msr = XMLHelper.Deserialize<MediaStreamingResult>(PostObjects);
                    //if (msr == null)
                    //{
                    //    XMLresponse = XMLHelper.Serialize<string>("failure");
                    //}
                    StreamingManager.Default.store(msr);
                }
                else if (action.StartsWith("xml/checkexists"))
                {
                    ...
                    bool CheckInBackgroundTranscodedDirectory = action.Contains("streamtranscodedinbackground64");

                    if (CheckInBackgroundTranscodedDirectory)
                    {
                        int pos = action.IndexOf("streamtranscodedinbackground64", 0);
                        string action2 = action.Replace(action.Substring(0, "streamtranscodedinbackground64".Length), "");
                        ...
                        ID = StreamingManager.Default.MediaStreamingResultsContains(filePath);
                        // Int32.TryParse(action2, out ID);
                        //                        action = "segment-0.ts";
                        string segmentfile = "segment-0.ts";
                        string direc = Functions.AppDataFolder;
                        if (CheckInBackgroundTranscodedDirectory)
                        {
                            segmentfile = Path.Combine(direc,
                                                       "static\\BackgroundTranscodedMediastreams\\" + ID + "\\" +
                                                       segmentfile);
                        }
                        if (File.Exists(segmentfile))
                        {
                            XMLresponse = XMLHelper.Serialize<string>("" + ID);
                        }
                        else
                        {
                            XMLresponse = XMLHelper.Serialize<string>("failure");
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            //Hack: cuz all clients only ask for 2 segments, actually we need 3, so +1:
                            //           action = parts[0] + "-" + (Int32.Parse(parts2[0]) + 1) + ".ts";


                            action = parts[0] + "-" + Int32.Parse(parts2[0]) + ".ts";

                            files = di.GetFiles(action);
                            try
                            {
                                if (files != null && files.Length > 0 && files[0].Exists)
                                // && files[0].Length>1000)            //also check if has grown a bit, needed for slow servers!
                                {
                                    XMLresponse = XMLHelper.Serialize<string>("success");
                                }
                                else
                ...
                else if (action.StartsWith("xml/livetv"))
                {
                    ...
                    try
                    {
                        serviceId = action.Substring(0, action.IndexOf("uniqueandroid="));
                    }
                    catch (Exception e)
                    {
                        serviceId = "unknown";
                    ...
                    try
                    {
                        uniqueAndroidId = action.Substring(0, action.IndexOf("length="));
                    }
                    catch (Exception e)
                    {
                        uniqueAndroidId = "unknown";
                    }
                    ...
                    try
                    {
                        servId = long.Parse(serviceId);
                        // Create a new recording request
                        newRR = new RecordingRequest(tryStartTime.ToUniversalTime(), servId, tryDuration,
                                                     filenameId);
                    }
                    catch (Exception e)
                    {
                        failedValidation = true;
                        failedValidationReason = "Service id not valid: " + serviceId;
                    }
                    // Passed validation?
                    if (failedValidation)
                    {
                    ...
                    else
                    {
                        ...
                        if (RecordFromQueue(out txtRecSummary2))
                        {
                            ...
                            if (txtRecSummary2.Contains("The scheduling was successful"))
                            {
                                ...
                                {
                                    if (Settings.Default.DebugAdvanced)
                                        Functions.WriteLineToLogFile(RecordFromQueueResult);
                                    if (!RecordFromQueueResult.StartsWith("Recording not scheduled"))
                                    {
                                        ...
                                        do
                                        {
                                            ...
                                            if (
                                                DateTime.Compare(begin.AddSeconds(waittimeforfiletoappear), DateTime.Now) <
                                                0) //not found after 20 seconds
                                            {
                                                if (Settings.Default.DebugAdvanced)
                                                {
                                                    foreach (string rectvfolder in Settings.Default.RecordedTVFolders)
                                                    {
                                                        ...
                                                        {
                                                            Functions.WriteLineToLogFile("However, this does exist :" +
                                                                                         fp);
                                                        }
                                                    }
                                                }
                            ...
                            if (!found)
                            {
                                ...
                                List<RPRecording> recsToday = EPGManager.AllRecordingsRunningInTimeFrame(dateRange, true);
                ...
                else if (action.StartsWith("xml/islivetvstarted/"))
                {
                    string strStreamerID = action.Replace("xml/islivetvstarted/", "");
                    int streamerID = 0;
                    if (int.TryParse(strStreamerID, out streamerID))
                    {
                    }
                    ...
                    if (StreamingManager.Default.LiveTVStarted(streamerID))
                    {
                        XMLresponse = XMLHelper.XMLReponseWithOutputString("started");
                    }
                    else
                    {
                        XMLresponse = XMLHelper.XMLReponseWithOutputString("No Data");
                ...
                else if (action.StartsWith("xml/getmappings"))
                {
                    string WorkingDirectory = action.Replace("xml/getmappings/", "");
                    MediaStreamingRequest msr = XMLHelper.Deserialize<MediaStreamingRequest>(PostObjects);
                    FFMPGProber prober = new FFMPGProber();
                    string mapping;
                    while (true)
                    {
                        FFMPGProber prober2 = new FFMPGProber();
                        bool result = prober2.Probe("ffmpeglatest.exe", "", Functions.ToolkitFolder, msr.InputFile, WorkingDirectory, msr.UseAudioStreamIndex);
                        mapping = prober2.mapArguments.ToString();
                        if (string.IsNullOrEmpty(mapping)) // May take a small while
                        {
                            Functions.WriteLineToLogFile("Probing failed, trying again");
                        }
                        else
                        {
                            Functions.WriteLineToLogFile("Success! Mappings are :" + mapping);
                            break;
                        }
                    }
                    XMLresponse = XMLHelper.XMLReponseWithOutputString(mapping);
                    ...
                    try
                    {
                        if (int.TryParse(strStreamerID, out streamerID))
                        {
                            if (!StreamingManager.Default.StopStreamer(streamerID, 5))
                                Functions.WriteLineToLogFile(
                                    "Warning in stopping mediastream: Could not stop streamer ID " + strStreamerID);
                        }
                        else
                        {
                            Functions.WriteLineToLogFile(
                                "Warningin stopping mediastream2: Could not parse streamer ID " + strStreamerID);
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                //else if (action.StartsWith("xml/startrunners"))
                //{
                //    action = action.Replace("xml/startrunners/", "");
                //    int ID = 0;
                //    try
                //    {
                //        Int32.TryParse(action, out ID);
                ...
                //    string txtError = "";
                //    StreamingManager.Default.SegmentFromStreamer(0, ID, 1, ref TSdata, ref txtError, true); //start up the runners
                //}
                else
                {


                    Functions.WriteLineToLogFile("XML: Unknown request : " + action);
                    XMLresponse = "";
                }
        ...
        //public static Dictionary<int, int> segstarts = new Dictionary<int, int>(); //reset to null when streamids cleaned up by janitor
        ...
        private string WebSvcDeleteFileByFilePath(string filename)
        {
            ...
            if (filename != null && (!fileNameStartsWithAnyRecordedTVFolder(filename) && filename.StartsWith("!")))
            {
                if (filename.Length > 1)
                {
                    filename = filename.Substring(1, filename.Length - 1);
                }
                Functions.WriteLineToLogFile("Error - Deletion of '" + filename + "' not in Recorded TV Folder. Now trying all Recording folders");
                string FilenameToTest = "";
                bool found = false;
                foreach (string recTVFolder in Settings.Default.RecordedTVFolders)
                {
                    FilenameToTest = recTVFolder + "\\" + filename;
                    Functions.WriteLineToLogFile("looking for " + FilenameToTest);
                    string[] files = System.IO.Directory.GetFiles(recTVFolder, filename + "*.wtv", System.IO.SearchOption.TopDirectoryOnly);
                    foreach (string f in files)
                    {
                        ...
                        if (Settings.Default.DebugAdvanced)
                        {
                            Functions.WriteLineToLogFile(f + " Found!");
                        ...
                        {
                            Functions.WriteLineToLogFile("could not delete " + f);
                        }
                    }
                }
                if (!found)
        ...
        private void waitForCopyToComplete(string segfile)
        {
            ...
            while (true)
            {
                try
                {
                    using (System.IO.File.Open(segfile, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        break;
                    }
                }
        ...
        private void SendSegmentData(string txtAction, ref BrowserSender browserSender, int ID, bool sendToBrowser, bool doTranscode)
        {
            ...
            if (txtAction.StartsWith("seg$")) //first wait screen segment for latest FFMPEG
            {
                LatestFFmpeg = true;
        ...
        private bool TryManualRecording()
        {
            ...
            else
            {
                qsParams.Add("queueid", RecordingQueue.AddToQueue(newRR));
                string foo;
                return RecordFromQueue(out foo);
            }
        ...
        private bool RecordFromQueue(out string txtRecordingSummary)
        {
            ...
            if (Settings.Default.DebugAdvanced)
                Functions.WriteLineToLogFile("Now trying scheduleRecordingForHTML " + newRR);
            bool recResult = scheduleRecordingForHTML(newRR, out txtRecSummary);
            if (Settings.Default.DebugAdvanced)
                Functions.WriteLineToLogFile("Now trying scheduleRecordingForHTML result: " + txtRecSummary + " " + recResult);
            // Report the result
            txtResponse += recResult ? "<p class='recordsuccess'>" : "<p class='recordfailure'>";
            spoolMessage(txtRecSummary); // to screen / logfile
            RecordFromQueueResult = txtRecSummary;
            txtRecordingSummary = txtRecSummary;
