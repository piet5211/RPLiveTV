...
namespace RemotePotatoServer
{
    public static class Functions
    {
        ...
        static Functions()
        {
            ...
            StoredLogEntries2 = new List<string>();
            StoredLogEntries3 = new List<string>();
            StoredLogEntries4 = new List<string>();
        }

        // Errors
        static List<string> StoredLogEntries;
        static List<string> StoredLogEntries2;
        static List<string> StoredLogEntries3;
        static List<string> StoredLogEntries4;
        ...
        public static string DebugLogFileFN2
        {
            get
            {
                string strPath = AppDataFolder;
                return Path.Combine(strPath, "RPServerLiveTVEncoder.log");
            }
        }
        public static string DebugLogFileFN3
        {
            get
            {
                string strPath = AppDataFolder;
                return Path.Combine(strPath, "RPServerFFMpegCopy.log");
            }
        }
        public static string DebugLogFileFN4
        {
            get
            {
                string strPath = AppDataFolder;
                return Path.Combine(strPath, "RPServerDuration.log");
            }
        }
        static object writeLogLock = new object();
        static object writeLogLock2 = new object();
        static object writeLogLock3 = new object();
        static object writeLogLock4 = new object();
        ...
        public static void WriteLineToLogFile4(string txtLine)
        {
            Monitor.Enter(writeLogLock4);
            string logLine = System.String.Format("{0:O}: {1}.", System.DateTime.Now, txtLine);

            System.IO.StreamWriter sw;
            try
            {
                sw = System.IO.File.AppendText(DebugLogFileFN4);
            }
            catch
            {
                // Store the log entry for later
                if (StoredLogEntries4.Count < 150)  // limit
                    StoredLogEntries4.Add(logLine);

                Monitor.Exit(writeLogLock4);
                return;
            }
            ...
            try
            {
                ...
                if (StoredLogEntries4.Count > 0)
                {
                    foreach (string s in StoredLogEntries4)
                    {
                        sw.WriteLine(s);
                    }
                    StoredLogEntries4.Clear();
                }
        ...
        public static void WriteLineToLogFile3(string txtLine)
        {
            Monitor.Enter(writeLogLock3);
            string logLine = System.String.Format("{0:O}: {1}.", System.DateTime.Now, txtLine);

            System.IO.StreamWriter sw;
            try
            {
                sw = System.IO.File.AppendText(DebugLogFileFN3);
            }
            catch
            {
                // Store the log entry for later
                if (StoredLogEntries3.Count < 150)  // limit
                    StoredLogEntries3.Add(logLine);

                Monitor.Exit(writeLogLock3);
                return;
            }
            ...
            {
                // Write any pending log entries
                if (StoredLogEntries3.Count > 0)
                {
                    foreach (string s in StoredLogEntries3)
                    {
                        sw.WriteLine(s);
                    }
                    StoredLogEntries3.Clear();
                }
        ...
        public static void WriteLineToLogFile2(string txtLine)
        {
            Monitor.Enter(writeLogLock2);
            string logLine = System.String.Format("{0:O}: {1}.", System.DateTime.Now, txtLine);

            System.IO.StreamWriter sw;
            try
            {
                sw = System.IO.File.AppendText(DebugLogFileFN2);
            }
            catch
            {
                // Store the log entry for later
                if (StoredLogEntries2.Count < 150)  // limit
                    StoredLogEntries2.Add(logLine);

                Monitor.Exit(writeLogLock2);
                return;
            }
            ...
            {
                // Write any pending log entries
                if (StoredLogEntries2.Count > 0)
                {
                    foreach (string s in StoredLogEntries2)
                    {
                        sw.WriteLine(s);
                    }
                    StoredLogEntries2.Clear();
                }
        ...
        */
        ...
        public static string BackgroundStreamBaseFolder
        {
            get
            {
                string ADF = AppDataFolder;
                return Path.Combine(ADF, "static\\BackgroundTranscodedMediastreams\\");
            }
        }
