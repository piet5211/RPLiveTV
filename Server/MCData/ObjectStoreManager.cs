...
namespace CommonEPG
{
    public class ObjectStoreManager : IDisposable
    {
        ...
        public void ScheduleRecording(RecordingRequest rr, EventWaitHandle _ewh)
        {
            ...
            // Too much padding
            if ((rr.Postpadding > 10800) && (rr.Prepadding > 10800))
            {
                ScheduleInitialFailureResult.ErrorMessage = "Pre or Post padding must be less than 300 minutes.";
                ScheduleInitialFailureResult.RequestResult = RecordingResult.RequestResults.FailedWithError;
                ScheduleRecordingCompleted();
                return;
            }
            ...
            DebugNormal("ObjectStoreManager: ScheduleRecording finished");
        }
        ...
        public Dictionary<string, TVService> GetAllServices(bool mergeLineUps, bool includeInternetTV, bool includeHiddenChannels, bool blockUserHidden, bool blockUserAdded, bool blockUserMapped, bool blockUnknown, bool DebugChannelList)
        {
            ...
            foreach (Channel c in channels)
            {
                ...
                if (!includeInternetTV)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to ct == ChannelType.WmisBroadband)");
                        allowChannel = false;
                    }
                ...
                if (!includeHiddenChannels)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to (c.UserBlockedState == UserBlockedState.Blocked) || (c.UserBlockedState == UserBlockedState.Disabled)");
                        allowChannel = false;
                    }

                    if (c.IsSuggestedBlocked)
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.IsSuggestedBlocked");
                        allowChannel = false;
                    }

                    if (c.Visibility == ChannelVisibility.NotTunable)
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.Visibility == ChannelVisibility.NotTunable");
                        allowChannel = false;
                    }

                    if (c.Visibility == ChannelVisibility.Blocked)
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.Visibility == ChannelVisibility.Blocked");
                        allowChannel = false;
                    }
                }
                ...
                if (blockUserHidden)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.ChannelType == ChannelType.UserHidden");
                        allowChannel = false;
                    }
                }
                ...
                if (blockUserAdded)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.ChannelType == ChannelType.UserAdded");
                        allowChannel = false;
                    }
                }
                ...
                if (blockUserMapped)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.ChannelType == ChannelType.UserMapped");
                        allowChannel = false;
                    }
                }
                ...
                if (blockUnknown)
                {
                    ...
                    {
                        if (DebugChannelList) DebugNormal("(channel " + c.CallSign + " not added due to c.ChannelType == ChannelType.Unknown");
                        allowChannel = false;
                    }
                }
