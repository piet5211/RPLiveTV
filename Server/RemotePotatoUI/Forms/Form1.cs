...
//88888
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            currentTabPage = LiveTVTab.SelectedTab;
        }
        // Form Control events
        ...
        void ShowHideTabPage(TabPage tpPage, bool showPage)
        {
            if (showPage)
            {
                if (!LiveTVTab.TabPages.Contains(tpPage))
                    LiveTVTab.TabPages.Add(tpPage);
            }
            else
            {
                if (LiveTVTab.TabPages.Contains(tpPage))
                    LiveTVTab.TabPages.Remove(tpPage);
            }
        }
        #endregion
        ...
        private void ToggleServer(bool runAsService)
        {
            if (ServerIsRunning)
            {
                ...
                //    catch (Exception)
                //    {
                //        //probably live tv not installed
                //        RPMessageBox.ShowAlert("If using Live TV: Could not stop Live TV Front-end, please stop it manually.");
                //    }
                //}
                StopWebServer();
            }
            else
            {
                ...
                //{
                //    LiveTV = new Process();
                //    LiveTV.StartInfo.FileName = "LiveTVUI.exe";
                //    try
                //    {
                //        LiveTV.Start();
                //    }
                //    catch (Exception)
                //    {
                //        RPMessageBox.ShowAlert("If using Live TV: Live TV Front-end not installed, please reinstall, see http://myfrem.nl.");
                //    }
                //}
            }
        }
        public void StartWebServer(bool startAsService)
        {
            if (ServerIsRunning)
            {
            ...
            // No URL reserved - ask first...  (then try to start server again)
            if (Settings.Default.UsingLiveTV &&
                (Settings.Default.LastSetSecurityForLiveTVPort != Settings.Default.RPPortWithLiveTV))
            {
            ...
            {
			//88888
                webserverTC = new ThreadController(this); // Create controller for webserver
//                webserverTC = new ThreadController(); // Create controller for webserver
        ...
        void StopWebServer()
        {
            ...
            if (ServerRunningType == ServerRunningTypes.ServiceRunning)
            {
                ...
                {
                    RPMessageBox.ShowAlert("Could not stop service, now forcing.");
                    Process[] workers = Process.GetProcessesByName("RemotePotatoService");
        ...
        {
            private RPWebServer rpwebserver;

            public Listener(RPWebServer rpwebserver)
            {
                this.rpwebserver = rpwebserver;
            }
            ...
            {
                ((Form1)this.rpwebserver.caller).StopWebServer();//probeer te traceren uit webserver naar dittum of een vglbare fie
                ((Form1)this.rpwebserver.caller).ToggleServer();
            }
        ...
        void ReserveURLForLiveTVPort(bool startServerAfterSuccessfulReservation)
        {
            URLReserver reserver = new URLReserver();
        ...
        {
            string target = "http://myfrem.nl/forum";
            System.Diagnostics.Process.Start(target);
        }
        ...
        void BindControls()
        {
            ...
            //this.cbEnableLiveTV.DataBindings.Add(new System.Windows.Forms.Binding("Checked", Settings.Default, "UsingLiveTV", true, System.Windows.Forms.DataSourceUpdateMode.OnPropertyChanged));
        ...
        SettingsProviderNet.SettingsProvider settingsSaver;
        ...
        private void linkApllytoVideosAsWell_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string txtHelper = "When checked: Piping can only be used for new live TV; GPU acceleration and number of CPUs will only be used when the latest ffmpeg is used (as in the Android (when use recent ffmpeg checked in expert settings), iOS, and windows 8.1 client).";
        ...
        {
            string strLTV = "Ticking this box allows you watch Live TV if you have Windows Media Center 7, 8 or 8.1 as media center.\r\n\r\nWould you like to read more about this now?";

            if (RPMessageBox.ShowQuestion(strLTV, "Live TV Support") == System.Windows.Forms.DialogResult.Yes)
            {
                string target = "http://myfrem.nl";
                System.Diagnostics.Process.Start(target);
