...
namespace RemotePotatoServer.Network
{
    public class NATHelper
    {
        ...
        public bool RPMappingsExist(string LocalIP)
        {
            ...
            foreach (PortMappingEntry portMapping in mappings)
            {
                if (
                    ...
                    (portMapping.InternalPort == Settings.Default.RPPortWithLiveTV)
                    )
                    foundPort1 = true;
              /*  else if (
                    (portMapping.InternalClient == LocalIP) &&
        ...
        {
            NatHelperReponseCodes foo = DeleteForwardingRule(Convert.ToInt32(Settings.Default.RPPortWithLiveTV), ProtocolType.Tcp);
            foo = DeleteForwardingRule(Convert.ToInt32(Settings.Default.SilverlightStreamingPort), ProtocolType.Tcp);
            foo = DeleteForwardingRule(Convert.ToInt32(Settings.Default.RPPortWithLiveTV), ProtocolType.Udp);
            ...
            NatHelperReponseCodes outputCode1 = ForwardPort(Convert.ToInt32(Settings.Default.RPPortWithLiveTV), LocalIP, ProtocolType.Tcp, "RPhttp");
